#!/usr/bin/env python3
"""Evaluate DDAD depth fine-tuning checkpoints for mix early-fusion models."""
...
def project_points_to_depth_targets(
    ...
    if frame_count <= 0 or seq_len % frame_count != 0:
        raise ValueError(f"Cannot align points to cameras: seq_len={seq_len}, frame_count={frame_count}")
...
def save_metrics_report(
    ...
    ckpt = Path(ckpt_path)
    report_path = ckpt.with_name(f"{ckpt.stem}_ddad_eval.txt")
...
def evaluate(
    ...
    with torch.inference_mode():
        for batch in bar:
            ...
            gt_depth = build_depth_supervision_target(metric_batch, supervision_source)
...
def parse_args(
    ...
    parser.add_argument("config", nargs="?", default=str(default_config_path), help="mmengine config file.")
