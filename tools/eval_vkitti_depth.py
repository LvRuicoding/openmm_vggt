...
The evaluation loop is shared with the KITTI script, but this entrypoint uses
the vKITTI config by default and reports dataset-specific labels in logs.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate VGGT on the vKITTI validation split."
    )
    parser.add_argument(
        "config",
        nargs="?",
...
def main() -> None:
    ...
    try:
        ...
        log("=" * 70)
        log("vKITTI Depth — validation split evaluation")
