...
from openmm_vggt.models.aggregator import Aggregator
from openmm_vggt.models.vggtfast.vggt.models.aggregator import Fast_Aggregator
...
from openmm_vggt.layers.utils.block_mv import Block_mv
from openmm_vggt.models.utils.seperate_camera_head import CameraHead_seperate
from torch.utils.checkpoint import checkpoint


@MODELS.register_module()
class VGGT_decoder(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None,others = None):
        ...
        if self.rope is not None:
            ...
            pos = pos + 1
            pos_special_mv = torch.zeros(B * S, 2, 2).to(images.device).to(pos.dtype)
            # pos_special_seq = self.position_getter(B * self.cam_num, 1, f_num, device=images.device).view(B*S,1,2)
            # pos_special_seq = pos_special_seq + 1
            pos = torch.cat([pos_special_mv, pos], dim=1) # 6 742 2
        ...
        for layer in range(layer_nums):
            ...
            _,_,T_num,_ = aggregated_tokens.size()
            aggregated_tokens = self.batch_norm(aggregated_tokens.reshape(-1,T_num,2048).permute(0,2,1)).permute(0,2,1).reshape(self.cam_num,-1,T_num,2048)
            ...
            for _ in range(self.depth):
                frame_tokens, atten_idx = self._process_mv_attention(frame_tokens,B,S,T_num-3,2048,atten_idx,pos)
            ...
            agg_camera_relative_tokens = self.camera_tokens_agg(agg_frame_tokens[...,0,:],'multiview') # 1 6 1 2048
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.camera_head is not None:
                ...
                predictions["scale"] = torch.mean(relative_pose_enc_T[:,1:,:]/predictions["mv_env"][:,1:,:][...,:3])
    ...
    def _process_mv_attention(self, tokens, B, S, P, C, global_idx, pos=None):
        ...
        if tokens.shape != (B, S * P, C):
            tokens = tokens.view(B, S, P, C).view(B, c_num,f_num, P, C)

        if pos is not None and pos.shape != (B, S * P, 2):
            pos = pos.view(B, S, P, 2).view(B, c_num,f_num, P, 2)
            
        for f_index in range(f_num):
            # query = tokens[:,:,f_index,:,:] # B N P C
            if f_index < atten_scale//2 :
                frame_range = torch.arange(0,atten_scale)
            elif f_index > f_num - atten_scale//2-1:
                frame_range = torch.arange(f_num-atten_scale,f_num)
            else:
                frame_range = torch.arange(f_index-atten_scale//2,f_index+(atten_scale+1)//2)
            rel_idx = f_index - frame_range[0]
            
            tokens_frame = tokens[:,:,frame_range,:,:]
            
            B,M,F,P,C = tokens_frame.size()
            tokens_frame = tokens_frame.reshape(B,-1,C)
            # print("q",query.size())
            # print("v",tokens_frame.size())
            
            # exist problem
            pos_frame = pos[:,:,frame_range,:,:]
            pos_frame = pos_frame.reshape(B,-1,2)
            
            # by default, self.aa_block_size=1, which processes one block at a time
            
            if self.training:
                query = checkpoint(self.mv_blocks[global_idx],tokens_frame, pos_frame,f_index, use_reentrant=False)
            else:
                query = self.mv_blocks[global_idx](tokens_frame, pos=pos_frame,q_idx=f_index)
            query = query.reshape(B,M,F,P,C)[:,:,rel_idx,:,:].unsqueeze(-3)
            token_list.append(query)
        tokens = torch.cat(token_list,dim=-3)
        # print(tokens.size())
        global_idx += 1
...
@MODELS.register_module(name="VGGT_fastdecoder")
class VGGT_fastdecoder(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024,
                 enable_camera=True, enable_point=True, enable_depth=True, enable_track=True,cam_num=6,
                 merging=0,
                 vis_attn_map=False):
        ...
        self.aggregator = Fast_Aggregator(
            img_size=img_size,
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None,others = None):
        ...
        last_pose_enc = aggregated_tokens_list[-1][...,0,:]# 6 9 2048
        # selected_list = [4, 11, 17, 23]
        # print(len(aggregated_tokens_list))
        # aggregated_tokens_list = [aggregated_tokens_list[i] for i in selected_list]
        layer_nums = len(aggregated_tokens_list)
