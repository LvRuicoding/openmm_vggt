...
from .mix_decoder_global_window_attn import ShiftWindowPatchVoxelCrossFusion
...
class mix_decoder_global_window_attn_early(mix_decoder_global):
    ...
    def _prepare_early_fusion_data(
        ...
        if voxel_features.shape[0] == 0:
            ...
            return {
                "voxel_tokens": voxel_features.new_zeros((0, self.early_voxel_feature_proj.out_features)),
        ...
        voxel_geometry = torch.stack(
            [
                u / max(float(image_w), 1.0),
                v / max(float(image_h), 1.0),
                depth / depth_scale.to(depth.dtype),
            ],
            dim=-1,
        ...
        flat_voxel_geometry = voxel_geometry.reshape(-1, 3)
        return {
            "voxel_tokens": flat_voxel_tokens[flat_visible]
            + self.early_voxel_geometry_proj(flat_voxel_geometry[flat_visible]),
            "flat_seq_ids": flat_seq_ids.reshape(-1)[flat_visible],
            "patch_y": patch_y.reshape(-1)[flat_visible],
            "patch_x": patch_x.reshape(-1)[flat_visible],
            "patch_h": patch_h,
            "patch_w": patch_w,
        }

    def _apply_early_window_fusion(
        self,
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None, others=None):
        ...
        patch_tokens = self._apply_early_window_fusion(patch_tokens, early_fusion_data)
