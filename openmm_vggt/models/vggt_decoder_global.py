...
class VGGT_decoder_global(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None,others = None):
        ...
        pose_tokens = real_world_extrinics_enc.reshape(B, self.cam_num, f_num, 7)
        if self.cam_num == 1:
            # For monocular sequences, each frame should keep its own pose token.
            relative_pose_enc = pose_tokens
        else:
            # For multi-camera rigs, camera-relative pose is assumed fixed across time.
        ...
        for layer in range(layer_nums):
            ...
            if relative_pose_enc.dim() == 4:
                frame_relative_tokens = relative_pose_enc.reshape(B, -1, 1, C)
            else:
                frame_relative_tokens = relative_pose_enc.unsqueeze(2).expand(B, self.cam_num, f_num, C).reshape(B, -1, 1, C)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.camera_head is not None:
                ...
                if self.cam_num == 1:
                    ...
                    zeros = torch.zeros((B, S, 1, 3), device=images.device, dtype=frame_extrinsic.dtype)
                    ones = torch.ones((B, S, 1, 1), device=images.device, dtype=frame_extrinsic.dtype)
                    homo_tail = torch.cat([zeros, ones], dim=-1)

                    relative_pose = relative_extrinsic.unsqueeze(2).expand(B, self.cam_num, f_num, 3, 4).reshape(B, -1, 3, 4)
                    relative_pose = torch.cat([relative_pose, homo_tail], dim=-2)
                    frame_pose = frame_extrinsic.unsqueeze(1).expand(B, self.cam_num, f_num, 3, 4).reshape(B, -1, 3, 4)
                    frame_pose = torch.cat([frame_pose, homo_tail], dim=-2)

                    predictions["extrinsic"] = relative_pose.matmul(frame_pose)[..., :3, :]
                    predictions["intrinsic"] = intrinsic.unsqueeze(2).expand(B, self.cam_num, f_num, 3, 3).reshape(B, -1, 3, 3)
