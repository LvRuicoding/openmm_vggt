...
from openmm_vggt.utils.pose_enc import pose_encoding_to_extri_intri
from openmm_vggt.utils.geometry import closed_form_inverse_se3
from openmm_vggt.utils.load_fn import load_and_preprocess_images,load_and_preprocess_images_pos
...
@MODELS.register_module()
class VGGT_rel(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024,
                 enable_camera=True, enable_point=True, enable_depth=True, enable_track=True):
        super().__init__()
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None,others = None):
        ...
        relative_pose_enc = real_world_extrinics_enc.reshape(B,cam_num,f_num,7)[:,:,0,:] # B 6 7 (4+3)
        ...
        relative_pose_enc = self.layer_norm(relative_pose_enc).view(B,cam_num,1,2048) # B 6 2048
        relative_pose_enc = relative_pose_enc.expand(B,cam_num,f_num,2048).reshape(B,-1,2048)
        for index in range(len(aggregated_tokens_list)):
           aggregated_tokens_list[index][:,:,0,:] += relative_pose_enc
