...
try:
    ...
    _mask_mod_signature = Callable[..., Tensor]
    flex_attention = None
...
class Attention_attenmap(nn.Module):
    def __init__(
        ...
        if FLEX_ATTENTION_AVAILABLE:
            ...
            self.opt_flex_attention = partial(flex_attention, block_mask=self.block_mask)
            if hasattr(torch, "compile"):
                self.opt_flex_attention = torch.compile(self.opt_flex_attention)
            self.kernel_options = {
                "BLOCK_M": 64,
                "BLOCK_N": 64,
                "BLOCK_M1": 32,
            ...
            warnings.warn(
                "torch.nn.attention.flex_attention is unavailable; "
    ...
    def forward(self, x: Tensor, pos=None,q_idx=None) -> Tensor:
        ...
        if self.opt_flex_attention is not None:
            x = self.opt_flex_attention(
